...
(defn init_table []
  (let [table (vec (repeat 4 (vec (repeat 4 0)))),
        i (rand-int 4),j (rand-int 4)]
    (assoc table i (assoc (nth table i) j 2))))

(defn shift [col]
  (loop [result col i 0]
    (if (>= i (- (count result) 1)) result
      (let [tmp (nth result i),tmp_n (nth result (inc i)),
...
(defn rot [table n]
  (loop[result table i 0]
    (let [to_vec_rev #(vec (reverse %)),
      ...
      (if(>= i n)(vec (map vec result))(recur (rotation result)(inc i)))))) 
...
(defn set_random [table]
  (let [grid_list (for [x (range 0 4) y (range 0 4)] [x y]),
        ...
        emp_x (first emp_grid),emp_y (second emp_grid),
        rand_v (* (+ (rand-int 2) 1) 2)]
   (assoc table emp_y (assoc (nth table emp_y) emp_x rand_v))))

(defn update_table [table command]
  (let [direction #(val (find (hash-map :Left 0,:Down 1,:Right 2,:Up 3) %))]
    (set_random (move table (direction command)))))
...
(defn -main
  [& args]
  (loop[table (init_table)]
    (output table)
     (let [dir (keyword (read-line)),new_table (update_table table dir)]
      (if (= table new_table)(println "Game Over!!")(recur new_table)))))
